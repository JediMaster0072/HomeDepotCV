...
if [[ -d "$PROJECT_ROOT/predictions_json_export" ]]; then
  echo "Syncing per-image pipeline predictions..."
  "${RSYNC[@]}" \
    "$PROJECT_ROOT/predictions_json_export/" \
    "${USER}@${HOST}:~/HomeDepotCV/predictions_json_export/"
elif [[ -f "$PROJECT_ROOT/predictions.json" ]]; then
  echo "Syncing legacy pipeline OCR predictions..."
  "${RSYNC[@]}" \
    "$PROJECT_ROOT/predictions.json" \
    "${USER}@${HOST}:~/HomeDepotCV/predictions.json"
fi

if [[ -d "$PROJECT_ROOT/Golden_Dataset_overhead_eval_expected_sku" ]]; then
  echo "Syncing model-review source image/JSON pairs..."
  "${RSYNC[@]}" \
    --include '*/' --include '*.jpg' --include '*.jpeg' --include '*.png' \
    --include '*.json' --exclude '*' \
    "$PROJECT_ROOT/Golden_Dataset_overhead_eval_expected_sku/" \
    "${USER}@${HOST}:~/HomeDepotCV/Golden_Dataset_overhead_eval_expected_sku/"
fi
