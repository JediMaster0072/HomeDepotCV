...
def build_rows(args: argparse.Namespace, prior_rows: list[dict]) -> list[dict]:
    ...
    for image_path, json_path in iter_image_json_pairs(args.dataset):
        ...
        for shape_idx, shape in enumerate(data.get("shapes", [])):
            ...
            scorability = str(shape.get("expected_sku_scorability", "") or "").strip()
            digit_class = str(shape.get("expected_sku_digit_class", "") or "").strip()
            ...
            if prior:
                if is_reviewed_expected_sku(prior.get("expected_sku", "")):
                    ...
                    scorability = prior.get("scorability", "").strip() or scorability
                    digit_class = prior.get("sku_digit_class", "").strip() or digit_class
            ...
            rows.append(
                {
                    ...
                    "review_status": review_status or (
                        "not_applicable"
                        if is_na_expected_sku(expected_sku)
                        else "reviewed" if expected_sku else "needs_review"
                    ),
                    "scorability": scorability or scorability_from_expected_sku(expected_sku),
                    "sku_digit_class": digit_class or sku_digit_class(expected_sku),
                    "reviewer": reviewer,
                    "notes": notes,
                    "bbox_x1": x1,
                    "bbox_y1": y1,
