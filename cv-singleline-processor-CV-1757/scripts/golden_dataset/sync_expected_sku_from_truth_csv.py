    ...
    with truth_csv.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if is_na_expected_sku(raw_expected):
                if not notes:
                    continue
                expected_sku = "X" if raw_expected.upper() == "X" else "N/A"
            ...
            rows[key] = {
                ...
                "scorability": row.get("scorability", "").strip()
                or scorability_from_expected_sku(expected_sku),
                "sku_digit_class": row.get("sku_digit_class", "").strip()
                or sku_digit_class(expected_sku),
...
def update_dataset(dataset: Path, truth_rows: dict[tuple[str, int, str], dict], dry_run: bool) -> tuple[int, int, int]:
    ...
    for json_file, file_rows in sorted(by_file.items()):
        ...
        for (file_name, shape_idx, label), values in file_rows:
            ...
            shape["expected_sku_scorability"] = values["scorability"]
            shape["expected_sku_digit_class"] = values["sku_digit_class"]
