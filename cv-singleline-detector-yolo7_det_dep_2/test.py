...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default="/home/harish.k/Projects/Home-Depot/YOLO-V7-NEW/yolov7/runs/train/yolov7_hw_singleline_nosku_1024_2026-04-17/weights/best.pt",
        help="model.pt path(s)",
    )
    parser.add_argument(
        "--data",
        type=str,
        default="/home/harish.k/Projects/Home-Depot/new_dataset_copy/Single_line/stratified_output_dataset_set2/no_sku/data.yaml",
        help="*.data path",
    )
    ...
    parser.add_argument("--name", default=f"yolov7_singleline_hw_nosku_{datetime.now().strftime('%Y-%m-%d')}", help="save to project/name")
    parser.add_argument("--exist-ok", default=True, action="store_true", help="existing project/name ok, do not increment")
    parser.add_argument("--no-trace", action="store_true", help="don`t trace model")
    parser.add_argument("--v5-metric", default=True, action="store_true", help="assume maximum recall as 1.0 in AP calculation")
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        for w in opt.weights:
            ...
            for i in x:  # img-size
                print(f"\nRunning {f} point {i}...")
                r, _, t = test(opt.data, w, opt.batch_size, i, opt.conf_thres, opt.iou_thres, opt.save_json, plots=False, v5_metric=opt.v5_metric)
