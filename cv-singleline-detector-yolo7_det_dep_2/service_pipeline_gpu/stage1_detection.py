...
class Stage1Detection:
    ...
    def run_inference(self, image_bgr: np.ndarray) -> List[Dict]:
        ...
        pred = non_max_suppression(raw, self._conf, self._iou, classes=None, agnostic=False)
        t3 = time_synchronized()

        logger.info(f"[Stage1] inference {(t2 - t1) * 1e3:.1f}ms  NMS {(t3 - t2) * 1e3:.1f}ms")
    ...
    def _associate_children(
        ...
        results = [{"label_bbox": lb["bbox"], "label_score": lb["confidence"], "children": []} for lb in labels]

        multi_match = unmatched = 0
        for se in sub_elements:
            candidates = [idx for idx, lb in enumerate(labels) if self._is_child_of(se["bbox"], lb["bbox"])]
            if not candidates:
                unmatched += 1
                continue
            if len(candidates) > 1:
            ...
            best = max(
                candidates,
                key=lambda idx: self._child_parent_score(se["bbox"], labels[idx]["bbox"], labels[idx]["confidence"]),
            )
    ...
    def _overlap_ratio(child_bbox: List[float], parent_bbox: List[float]) -> float:
        ...
        if ix1 >= ix2 or iy1 >= iy2:
            return 0.0
        child_area = max((child_bbox[2] - child_bbox[0]) * (child_bbox[3] - child_bbox[1]), 1e-6)
